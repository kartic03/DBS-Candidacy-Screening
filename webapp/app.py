...
def generate_groq_report(disease_duration, updrs3_total, hoehn_yahr, total_asym,
    ...
    try:
        ...
        system_prompt = (
            "You are a board-certified movement disorder neurologist. You are describing the "
            "output of a research model that predicts RECORDED DBS STATUS, that is, whether a "
            "patient resembles those who have already received deep brain stimulation. The model "
            "does NOT assess DBS candidacy, referral suitability, or surgical eligibility. "
            "Generate a report with exactly these five labelled sections, each on its own line, "
        ...
        # The Methods state the report is constrained to 120-160 words, and the
        # model reliably undershoots when asked politely. Enforce it: one
        # expansion pass, keeping the section structure and the SHAP directions.
        if len(report.split()) < 120:
            messages += [
                {"role": "assistant", "content": report},
                {"role": "user", "content":
                 f"That report is {len(report.split())} words. Rewrite it at 120 to 160 "
                 "words, keeping the same five section labels, the same facts and the "
                 "same feature directions. Add clinical detail rather than filler."},
            ]
            ...
            if 120 <= len(candidate.split()) <= 170:
                report = candidate
...
with gr.Blocks(
    ...
    <div class="app-footer">
        ...
        Seven pre-registered clinical features (MDS-UPDRS, H&amp;Y, disease duration, asymmetry)<br>
