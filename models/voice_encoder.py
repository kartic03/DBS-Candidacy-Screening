#!/usr/bin/env python3
"""Simple MLP encoder for voice / speech features.

JBI DBS Screening Project — Voice Modality
...
class VoiceEncoder(nn.Module):
    ...
    input_dim : int
        ...
        Default 88; UCI Voice has 22, UCI UPDRS has 36.
    classify : bool
    ...
    def __init__(self, input_dim: int = 88, classify: bool = False):
        super().__init__()
    ...
    def get_embedding_dim() -> int:
        return VoiceEncoder.EMBEDDING_DIM


# ── CLI smoke-test ───────────────────────────────────────────────────────────
if __name__ == "__main__":
    for dim_name, dim_val in [("default", 88), ("UCI_Voice", 22), ("UCI_UPDRS", 36)]:
        model = VoiceEncoder(input_dim=dim_val, classify=False).to(device)
        n_params = sum(p.numel() for p in model.parameters())
        print(f"[VoiceEncoder  input_dim={dim_val} ({dim_name})] Parameters: {n_params:,}")
    ...
    model_cls = VoiceEncoder(input_dim=88, classify=True).to(device)
    out_cls = model_cls(torch.randn(4, 88, device=device))
    assert out_cls.shape == (4, 2), f"Classifier head: expected (4, 2), got {out_cls.shape}"
    print("\n[VoiceEncoder classify=True] Logits shape:", out_cls.shape)
